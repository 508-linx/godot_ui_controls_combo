...
@export var title_prefix := '':
	set(new_value): title_prefix = new_value; __update_text();
@export var title := 'Status Name':
	set(new_value): title = new_value; __update_text();
@export var title_surfix := '':
	set(new_value): title_surfix = new_value; __update_text();
...
@export var title_color := Color.LIGHT_GRAY:
	set(new_value): title_color = new_value; __update_font();
@export var title_size := 16:
	set(new_value): title_size = new_value; __update_font();
@export var title_halign := HORIZONTAL_ALIGNMENT_LEFT:
	set(new_value): title_halign = new_value; __update_font();
@export var title_valign := VERTICAL_ALIGNMENT_TOP:
	set(new_value): title_valign = new_value; __update_font();
...
func __realign():
	...
	for node_name in [ 'space_0', 'node_title_prefix', 'node_title', 'node_title_surfix' ]:
